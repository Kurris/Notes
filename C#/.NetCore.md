...
- 安装`EF.Tool`用于`Migration`

- 安装`EF.SqlServer`/`Pomelo.MySql`用于使用扩展数据库
- 使用方法
  - 创建`MyDbContext` 继承`DbContext`

  - 属性`DbSet<T>`用于跟踪当前实体对象

  - 方法`OnConfiguring`用于配置`DbContext`相关

  - 方法`OnModelCreating`用于实体生成时,给定约束或者数据
  ...
  - e.g: 这种方式需要在`Startup`中注册`DbContext`服务,好处是配置连接字符串可以通过`appsetting.json`获取
        ```C#
...
### 深入配置
- 将类继承`IEntityTypeConfiguration<T>`接口,实现`Configure`,并在`OnModelCreating`中按顺序调用
	```C#
...
### 数据迁移`Migration`
- 在`Visual Studio` 中的**程序包管理器控制台**中
	```Shell
...
- 查询数据
	- 查询一行数据
		>如果查询的数据实体存在于当前上下文中,那么查询的实体就会被上下文中的实体附加上,而不会执行实际的查询
